...
package com.bytepoets.sample.androidtesting.helper
...
class LiveDataValueCapture<T> {
    ...
    @Suppress("SwallowedException")
    suspend fun assertSendsValuesMatches(timeout: Long, vararg matcher: Matcher<in T>) {
        val listMatcher = Matchers.contains(*matcher)
        if (listMatcher.matches(values)) {
            return
        }
        try {
            withTimeout(timeout) {
